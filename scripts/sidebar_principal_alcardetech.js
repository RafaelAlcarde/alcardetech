(function () {
  ...
  const FEATURES = [
    ...
    { id: 'conversor_contatos',     label: 'Conversor de Contatos',  type: 'modal',  adminOnly: true,  url: 'https://rafaelalcarde.github.io/alcardetech/neofluxx-importer.html' },
  ...
  async function fetchPermissions() {
    ...
    try {
      const res = await fetch(
        SUPABASE_URL + '/rest/v1/tenants_permissoes?tenant_key=eq.' + tenantKey + '&select=template_builder,kpis,disparo_campanha,kanban,portal_disparo,anexar_imagens,etiquetar_contatos,conversor_contatos&limit=1',
        {
          headers: {
            'apikey': SUPABASE_KEY,
  ...
  function waitForSidebar() {
    return new Promise(function(resolve) {
      function check() {
        ...
        if (mainAside && mainAside.children[1] && mainAside.children[1].children[0]) {
          resolve();
        } else {
