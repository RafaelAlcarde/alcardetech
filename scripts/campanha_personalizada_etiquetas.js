/**
 * CW SheetCampaign — v5.9
 * Importa contatos via CSV no modal de campanhas do WhatsApp
 * e aplica uma etiqueta preservando as etiquetas existentes.
 *
 * Melhorias v5.9 (vindas do script de etiquetas + correções de rate limiting):
 * - Resumo final detalhado: criados / etiquetados / já tinham / erros
 * - Suporte a city e company_name no CSV
 * - Validação do nome da etiqueta (espaços + lowercase automático)
 * - Prévia do CSV antes de processar (quantidade de contatos)
 * - Fluxo de dois passos: preenche → confirma → processa
 * - Rate limiting corrigido: batch 2 / 150ms no tier baixo
 * - Retry com backoff nos erros individuais de contato
 * - cancelFlag local à execução
 * - ensureLabel com retry (3 tentativas)
 * - findContactByPhone com validação de telefone exato
 * - Erros de API com status HTTP + body
 */
(function () {
  if (window.__cwSheetCampaign_v59) return;
  window.__cwSheetCampaign_v59 = true;

  // --- TRAVA DE SEGURANÇA ---
  const ALLOWED_ACCOUNTS = [];

  const log  = (...a) => console.log("[CW SheetCampaign v5.9]", ...a);
  ...
  if (!currentAccount) {
    log("Account ID ainda não detectado. Aguardando rota...");
  } else if (!isAllowedAccount()) {
    log(`Account ID ${currentAccount} não autorizado.`);
  } else {
  ...
  // ensureLabel com retry (3 tentativas com backoff)
    ...
    for (let attempt = 1; attempt <= retries; attempt++) {
      try {
        ...
        let found = Array.isArray(list)
          ? list.find(l => (l.title || l.name) === title)
          : null;
        if (!found) {
        ...
        log(`ensureLabel tentativa ${attempt} falhou, aguardando...`);
  ...
  // Retry com backoff para operações de contato individual
  ...
  function validateCSV(text) {
    ...
    if (dataRows.length > 500) {
      warnings.push(`⚠️ Atenção: ${dataRows.length} contatos é bastante. Recomendo dividir.`);
    }
  ...
  function isWhatsAppCampaignsPage() {
    ...
    if (!path.includes("/campaigns")) return false;
    const pageHasTitle = Array.from(document.querySelectorAll("h1,h2,h3,div,span,p"))
      .some(el => (el.textContent || "").trim().toLowerCase() === "campanhas do whatsapp");
    const sidebarHasWhatsapp = Array.from(document.querySelectorAll("a,button,div,span"))
      .some(el => (el.textContent || "").trim().toLowerCase() === "whatsapp");
    return pageHasTitle && sidebarHasWhatsapp;
  }
  ...
  function findPublicoBlock(modal) {
    ...
    if (el2) return el2.closest("div") || el2;
    return null;
  }
  ...
  function bindWrapLogic(wrap) {
    ...
    function checkForm() {
      ...
      const hasFile  = contactsToProcess.length > 0;
    ...
    fileInput.addEventListener("change", async () => {
      ...
      contactsToProcess = dataRows.map((r, idx) => {
          ...
          try {
            await withRetry(async () => {
              ...
              if (existing?.id) {
                ...
                // Atualiza city/company_name se fornecidos
                const attrs = {};
                if (c.city)         attrs.city         = c.city;
                if (c.company_name) attrs.company_name = c.company_name;
                if (Object.keys(attrs).length) {
                  try {
                    await api(`contacts/${contactId}`, "PUT", { additional_attributes: attrs });
                  } catch (e) {
                    log(`Aviso: falha ao atualizar atributos de ${c.phone_number}: ${e.message}`);
                  }
                }
              } else {
                const newContact = {
  ...
  async function injectIntoModal() {
    ...
    if (!modal) {
      alert("Não consegui localizar o modal de campanha na tela.");
      return;
    }
    const existingWrap = modal.querySelector("#cw-sheet-wrap");
    if (existingWrap) {
      existingWrap.scrollIntoView({ behavior: "smooth", block: "center" });
      log("Wrap já existe.");
      return;
    }
    ...
    if (publicoBlock && publicoBlock.parentElement) {
      ...
      log("Wrap inserido antes do bloco Público.");
    } else {
      const anchor = modal.querySelector("form") || modal;
      anchor.appendChild(wrap);
      log("Wrap inserido no final do modal.");
    }
  ...
  function removeTopButton() {
    document.getElementById("cw-personalizada-btn")?.remove();
  }
  ...
  function injectTopButton() {
    ...
    const native = findNativeCreateBtn();
    if (!native) return;
    if (document.getElementById("cw-personalizada-btn")) return;
  ...
  // ─── Observers ──────────────────────────────────────────────
  new MutationObserver(() => { injectTopButton(); })
  ...
  new MutationObserver(() => {
    ...
    if (url !== lastUrl) {
      lastUrl = url;
      setTimeout(() => { injectTopButton(); }, 300);
    }
